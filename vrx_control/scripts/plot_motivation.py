...
class ICRAMotivationFigure:
    ...
    def generate_panel_a(self, ax):
        ...
        if proposed_data and 'trajectory' in proposed_data:
            ...
            if len(x_rot) > 0:
                ...
                ax.plot([], [], 'g-', linewidth=2, label='EAMPC (Success)')
    ...
    def generate_panel_b(self, ax):
        ...
        ax.arrow(usv_x, usv_y, wind_fx, wind_fy,
                head_width=2.5, head_length=2, fc='darkorange', ec='darkorange',
        ...
        ax.text(-233, -581.5, '10 m', ha='center', fontsize=6)
    
    def generate_figure(self, save_path='icra_motivation_column.pdf'):
        """Generate the complete column-width figure"""
        ...
        fig = plt.figure(figsize=(3.5, 5.5))  # Taller to accommodate two panels
        
        # Create grid with adjusted spacing
        gs = GridSpec(2, 1, figure=fig,
                     height_ratios=[1.2, 1],  # Panel A slightly larger
                     hspace=0.25,
        ...
        self.generate_panel_a(ax1)
        
        # Panel B: Environmental forces
        ax2 = fig.add_subplot(gs[1, 0])
        self.generate_panel_b(ax2)
        
        # Save figure
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.savefig(save_path.replace('.pdf', '.png'), dpi=300, bbox_inches='tight')
        print(f"Column-width motivation figure saved to {save_path}")
